...
- Route entrypoint screens MAY live inside their own feature folder under `src/ui/screens/`.
- Feature-local support files like `messages.ts` and `*.util.ts` are allowed inside that feature folder.
- If a screen is split into additional UI states or auxiliary screens, those extracted files MUST live as direct children of `src/ui/screens/`, not nested inside the feature folder.
- Extracted screens MUST keep the `*.screen.tsx` suffix.
